...
- [ ] **`/api/templates` が 200 を返す**（ブラウザで直接開くか DevTools の Network で確認）
      → 500 なら `COSMOS_CONNECTION_STRING` 未設定を疑う
...
> 保存失敗は画面に出ません。**DevTools のコンソールにエラーが出ていないこと**も併せて確認してください。

---

## 5. 運用上の注意・将来課題

- **保存失敗が UI に出ない**：`COSMOS_CONNECTION_STRING` 未設定やネットワーク断でも、
  アプリは動いているように見えたまま保存だけが失敗します（例外はコンソールに出るのみ）。
  現状で最も実害の大きい既知の負債です。
