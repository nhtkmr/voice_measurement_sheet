...
品番＋品名＋工程の3つでテンプレを識別する（同じ品番でも工程が違えば別テンプレ）。**このキーがそのまま Cosmos のドキュメントIDになる**（`api/templates`）。

**文字制約**: キーは Cosmos のドキュメントID制約を受けるため、品番/品名/工程に `/ \ ? #`（Cosmos ID 禁止）・`␟`（区切り文字 SEP, U+241F）・制御文字・前後空白は使えない。これらを含むと upsert が失敗し、best-effort 同期の下では消失につながる。`templateFieldError()` が保存前にクライアントで弾く（サーバ挙動に依存せず防ぐ）。ハイフンや中間の空白は許可（例 `SAMPLE-001`）。
...
テンプレの書き込みも best-effort 同期だが、セッションと違い**起動時に `initTemplates()` がサーバ一覧で localStorage を全置換する**ため、サーバへ届かなかったローカルの作成/更新は次回起動で消えていた（データ消失バグ）。

対策として localStorage キー `vms.templates.pending`（`{ [key]: 'upsert' | 'delete' }`）で未同期の変更を追跡する:

- `saveTemplate`/`deleteTemplate` は成否を返し、失敗時に pending へ記録・成功時に除去。
- `initTemplates()` はサーバ一覧を反映する際、**pending な upsert はサーバに無くてもローカル値を残し、pending な delete はサーバに有っても除く**。反映後、pending を**再送**して成功分を落とす。
- 結果、サーバへ届かなかったテンプレは起動時に消えず、接続が戻れば自動で同期される。`/` 等の不正文字は §5-4 で弾かれるため、そもそも未同期に陥らない。
...
| 設定 | 表示名 | `moveNext()` の挙動 |
|---|---|---|
| `'item'`（既定） | 項目方向 | `col++` → 行末で `col=0; row++` → **最終行を超えたら留まる**（`return` でカーソル据え置き。行は自動追加しない） |
| `'row'` | No.方向 | `row++` → 最終行で `row=0; col=(col+1)%nCols` にラップ（**行は増えない**） |

どちらのモードも**行は自動で増えない**。本数は「本数」欄（`setRowCount`）と「＋1本追加」（`addRow`）で手動管理する。`movePrev()` も**先頭ではラップせずクランプ**する。以前は項目方向のみ最終セルで `addRow()` して無限に伸びたが、意図せず本数が増えるため据え置きに変更した。
...
| ファイル | 対象 |
|---|---|
| `template.test.ts` | 公差計算、複合キー、`templateLabel()`、`templateFieldError()`（不正文字）、**未同期テンプレの消失防止**（fetch モックで保存失敗→pending→initで残す/削除の未反映復活防止）、旧形式の自動移行、JSON往復、取込時の `upper` 再計算 |
...
- **`templateKey()` が2箇所に重複実装されている** — `src/template.ts` と `api/templates/index.js`。片方だけ変えるとクライアントとサーバでIDが食い違い、テンプレが二重登録される。**必ず両方を揃えること。** キーに使える文字は §5-4 参照（`/ \ ? # ␟` 等は禁止で、クライアントが `templateFieldError` で弾く）。
- エクスポート JSON の `schema` / `version` は**書き出すだけで、取り込み時に検証もマイグレーションもしていない**。将来 version 2 を作るなら移行経路をここに足す必要がある。

### 動作

- **取込 `mode: 'replace'` はサーバ側を削除しない**（コード中にも「簡易実装」と明記）。ローカルを全置換しても、削除分は次回 `initTemplates()` でサーバから復活する（未同期保護 §5-5 は「消えない」方向の保護であり、これは逆に「消したはずが戻る」方向で別問題）。なお取込の確認は **追記 / 全置換 / キャンセルの3択**で、Esc・キャンセルは何もしない（以前は Cancel が全置換に落ちる罠だった）。
